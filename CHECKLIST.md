...
> - **Triagem (Inbox) + página de detalhes da vaga:** enum `StatusVaga` ganhou `NOVA` e `DESCARTADA`; toda vaga raspada **nasce NOVA** (triagem), e o `GET /kanban` filtra NOVA/DESCARTADA (kanban = apenas vagas em processo). Novos: `GET /inbox` (template `inbox.html` com ações Visualizar / Adicionar ao Kanban → PATCH para `Ativa` / Descartar → PATCH para `Descartada`, removendo o card via `hx-swap="delete"`), `GET /vagas/{id}` (template `vaga_detalhes.html`: título, empresa, link `target="_blank"` para a Gupy, descrição, botão Voltar e placeholders para Análise de Perfil e Mock Interview) e `GET /configuracoes` (placeholder). Port `IVagaRepository` ganhou `buscar_por_id` (repositorio + `BuscarVagaPorIdUseCase` reexportando o mesmo `VagaNaoEncontradaError`). Menu atualizado nos 4 templates: 📋 Buscas / 📥 Inbox / 🗂️ Kanban / ⚙️ Configurações. TDD: 2 testes do use case, 2 de integração (`buscar_por_id`) e 4 de rotas (inbox filtrado, kanban sem NOVA/DESCARTADA, detalhes, 404). Suíte: **65 passed**. Smoke real: vaga 2 movida para NOVA → aparece só no Inbox, some do Kanban, detalhes e menu OK. Obs: índice de sistema do PostgreSQL do container foi corrompido e restaurado via `REINDEX SYSTEM` nos dois bancos.
> - **Refatoração de UX — fim do Inbox, Busca como motor e filtro de 30 dias:** Inbox removido (rota `GET /inbox` e template excluídos; menu agora [🔍 Busca] [🗂️ Kanban] [⚙️ Configurações]). Formulário/lista de buscas (apelido + URL) **migraram para `GET /configuracoes`** — único lugar que cria/remove buscas. `GET /` virou o **motor de operação**: dropdown com os apelidos das buscas salvas + botão Buscar → nova rota **`POST /buscar`** executa o orquestrador (injetável via `Depends(get_orquestrador)`) e devolve o partial `resultados_busca.html` com as vagas **NOVA** (cards com Visualizar / Adicionar ao Kanban via PATCH `hx-swap="delete"`). **Regra de negócio no scraper**: `extrair_vagas` ignora vagas publicadas há mais de 30 dias (`GupyScraper._dentro_do_prazo`, constante `DIAS_LIMITE_VAGA=30`; vagas sem data são mantidas). TDD: testes de rotas renomeados para `test_app.py` com 5 novos (dropdown, menu sem Inbox, configurações, POST /buscar feliz e busca inexistente) e 4 do filtro de prazo (incl. teste do loop com page/`_extrair_vaga_do_card` mockados). Suíte: **73 passed**. Smoke real: dropdown renderizado, configurações com formulário+lista, `/inbox` → 404.
> - **Ações nos cards e Limpar Kanban:** cada card do Kanban ganhou [👁 Visualizar] (`/vagas/{id}`), [↩️ Voltar p/ Busca] (PATCH → `Nova`, volta para triagem) e [🗑️ Descartar] (PATCH → `Descartada` com confirm), todos via HTMX `hx-swap="delete"`. Novo `LimparKanbanUseCase` (application/use_cases/limpar_kanban.py): lista todas e move para `DESCARTADA` apenas as em processo (`STATUS_EM_PROCESSO` — preserva NOVA e já-descartadas), retornando a quantidade. Rota **`POST /kanban/limpar`** retorna `HX-Refresh: true` para o HTMX recarregar o quadro; botão [🧹 Limpar Painel] com `hx-confirm` no topo do Kanban. TDD: 2 testes do use case (descarta só em processo / kanban vazio) e 2 de rota (HX-Refresh + descarte seletivo; ações e botão presentes no HTML). Suíte: **77 passed**. Smoke real: POST limpar descartou 11 vagas com `HX-Refresh: true` confirmado por consulta no PostgreSQL.
> - **Fix: Resiliência do scraper contra timeouts e cards sem footer:** o `_extrair_vaga_do_card` agora usa **`.text_content()`** (leitura instantânea do DOM, sem `timeout=2000`) para título, empresa, localização e footer — eliminando o `Locator.inner_text: Timeout 2000ms exceeded`; fallbacks via `.strip() or` garantem defaults sem vazio. O loop `extrair_vagas` envolve a extração de cada card em `try...except Exception as e`: com `print(f"Erro ao extrair card: {e}")` e `continue` — um card malformado (footer sem data) é pulado sem abortar a busca. Regex de data robusto com **grupo de captura** `r"(\d{2}/\d{2}/\d{4})"` e guarda `if not footer_texto` (vagas sem data → `None`, mantidas, sem quebrar). TDD: novo teste `test_extrair_vagas_deve_ignorar_card_malformado_e_continuar_com_os_demais` (um card lança timeout; o outro é extraído normalmente). Suíte: **78 passed**.
